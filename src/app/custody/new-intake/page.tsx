...
interface BailInfo {
  enabled: boolean; // toggles bail for this intake
  amount: number;
  type: BailType;
...
interface CustodyForm {
  id?: string; // if coming from ?from=CUST-...
  personName: string;
  personAge: number | '';
  chargesCsv: string; // comma separated for ease of input
  cellNumber: string;
...
/* ---------------- Helpers ---------------- */
const isoNow = () => new Date().toISOString().slice(0, 16); // for datetime-local
const digitsOnly = (s: string) => s.replace(/[^\d+ ]/g, '');

// receipt utils (print window + HTML)
...
const buildIntakeReceiptHTML = ({
  ...
  const chargesRows = charges
    .map((c, i) => `<tr><td style="width:48px">${i + 1}</td><td>${c}</td></tr>`)
...
export default function NewIntakeWizard() {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      setOfficer(
        u?.name || u?.fullName || u?.displayName || (u?.badgeId ? `Badge #${u.badgeId}` : 'Officer')
      );
    } catch {
      /* noop */
    }
  ...
  const removeProperty = (id: string) =>
    setForm((f) => ({ ...f, property: f.property.filter((p) => p.id !== id) }));
  ...
  // Print from Step 3 using current form state
  const generateAndPrintReceipt = () => {
    const number = makeReceiptNo('INTK');
    setReceiptNo(number);

    const charges = form.chargesCsv
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-sm text-gray-500">
            {fromId ? `Starting from record: ${fromId}` : 'New custody record'}
          </div>
        </div>
          ...
          <section className="rounded border bg-white p-4 space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, personAge: Number(e.target.value) || '' })
                }
              />
              <Input
                placeholder="Cell (e.g., A-3)"
            ...
            <Input
              ...
              value={form.chargesCsv}
              onChange={(e) => setForm({ ...form, chargesCsv: e.target.value })}
            />
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <select
                ...
                value={form.status}
                onChange={(e) =>
                  setForm({ ...form, status: e.target.value as CustodyStatus })
                }
              >
          ...
          <section className="rounded border bg-white p-4 space-y-4">
            ...
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, bail: { ...form.bail, enabled: e.target.checked } })
                }
              />
              ...
              <div className="grid md:grid-cols-3 gap-3">
                <Input
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      bail: { ...form.bail, amount: Number(e.target.value) || 0 },
                    })
                  }
                />
                <select
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      bail: { ...form.bail, type: e.target.value as BailType },
                    })
                  }
                >
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      bail: { ...form.bail, paymentMethod: e.target.value },
                    })
                  }
                >
                  <option>Cash</option>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, bail: { ...form.bail, guarantorName: e.target.value } })
                  }
                />
                <Input
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      bail: { ...form.bail, guarantorPhone: digitsOnly(e.target.value) },
                    })
                  }
                />
                <Input
                  ...
                  onChange={(e) =>
                    setForm({
                      ...form,
                      bail: { ...form.bail, guarantorAddress: e.target.value },
                    })
                  }
                />
              </div>
                ...
                <div key={p.id} className="rounded border p-3 space-y-3">
                  <div className="grid md:grid-cols-2 gap-3">
                    ...
                    <Input
                      ...
                      value={p.condition}
                      onChange={(e) => updateProperty(p.id, { condition: e.target.value })}
                    />
                    <label className="flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={p.isEvidence}
                        onChange={(e) => updateProperty(p.id, { isEvidence: e.target.checked })}
                      />
                      Mark as evidence
                    </label>
                    <Input
                      placeholder="Evidence Tag (if any)"
                      value={p.evidenceTag}
                      onChange={(e) => updateProperty(p.id, { evidenceTag: e.target.value })}
                    />
                    <Input
                      placeholder="Secure location"
                      value={p.secureLocation}
                      onChange={(e) => updateProperty(p.id, { secureLocation: e.target.value })}
                    />
                  </div>
        ...
        {step === 3 && (
          <section className="rounded border bg-white p-4 space-y-4">
            ...
            <p className="text-sm text-slate-600">
              Generate a print-ready receipt for this intake. A unique receipt number and
              timestamp will be embedded, with signature lines for the officer and detainee /
              guarantor.
            </p>
